...
- [```PowerStateControl```](https://github.com/Misha803/WOA-Tools/releases/tag/PowerStateControl)
...
### Примечания:
> [!Warning]
> Это действие полностью сотрёт данные **Android**, так что сделайте бэкап, если необходимо!
...
---
### Отключение гибернации и файла подкачки с помощью PowerStateControl
> [!NOTE]
> Вы можете пропустить этот шаг, если гибернация и файл подкачки уже отключены.

> [!Important]
> После изменения размера раздела настоятельно рекомендуется снова включить файл подкачки с помощью этой утилиты.

#### Используйте утилиту
- Запустите скачанную утилиту **`PowerStateControl_V2.0.exe`**.
- Нажмите переключатель **`Hibernation`**, чтобы отключить гибернацию.
- Нажмите переключатель **`Page File (Swap)`**, чтобы отключить файл подкачки.
- Нажмите кнопку **`APPLY`**.
- Нажмите кнопку **`EXIT`** и подтвердите перезагрузку, нажав **`REBOOT NOW`** в диалоге перезагрузки.
